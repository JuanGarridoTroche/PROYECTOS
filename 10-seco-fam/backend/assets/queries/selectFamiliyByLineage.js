  ...
  try {
    ...
    for(let family of data) {
      if(family.lineage === lineage) {
        user.push({id: family.id, lineage: family.lineage, active: family.active, pdf: family.pdf, password: family.password, url: family.url, role: family.role, logo: family.logo});        
      }
    }
