...
const createPDF = async (req, res, next) => {
  ...
  try {
    ...
    if (!url) {
      throw generateError(
        'Debes indicar a que familia quieres subir el pdf',
        400
      );
    }
    ...
    if (!req.files) {
      throw generateError(
        'Debes adjuntar un fichero en formato pdf para subir',
        404
      );
    }
    ...
    if (uploadPDF.mimetype !== 'application/pdf') {
      throw generateError(
        'El fichero adjunto no es un fichero pdf válido',
        403
      );
    }
    ...
    if (!lineageToChangePdf)
      throw generateError('No existe ese nombre de familia', 403);
