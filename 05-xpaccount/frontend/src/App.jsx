        ...
        if (token) {
          ...
          if (getAccounts.length > 0) {
            ...
            for (let i = 0; i < getAccounts.length; i++) {
              ...
              const calculatingBalance = readingEntries.reduce(
                (accumulator, current) =>
                  accumulator + parseFloat(current.amount),
                0
              );

              myAccountBalance.push({
                id: getAccounts[i].id,
                balance: calculatingBalance,
              });
            }
    ...
    <>
      ...
      <main>
        <Routes>
          ...
          <Route
            path="/account/:idAccount/category/:idCategory"
            element={<ReadSubcategories />}
          />
