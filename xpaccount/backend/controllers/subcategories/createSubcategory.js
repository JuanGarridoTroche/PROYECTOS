...
const createSubcategory = async (req, res, next) => {
  ...
  try {
    ...
    if (validationNameSubcat.error || validationNameSubcat === null) {
      throw generateError(
        "La subcategoría debe contener entre 1 y 100 caracteres. No puede quedar vacío."
      );
    }
    ...
    if (comment) {
      ...
      if (validationComment.error) {
        throw generateError(
          "El comentario no puede exceder de 192 caracteres."
        );
      }
    }
    ...
    const checkingNameSubcat = await selectSubcatByIdCatAndNameSubcatQuery(
      idCategory,
      nameSubcat
    );
